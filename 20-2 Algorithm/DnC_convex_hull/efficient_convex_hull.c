...
t_line *upper_hull(t_point *points, int num_point, t_point p1, t_point pn, t_line *lines, int *num_line, int *capacity)
{
	...
	{
		t_point p = points[i];
		float a = pn.y - p1.y, b = p1.x - pn.x, c = p1.x * pn.y - pn.x * p1.y;
		float d = distance(a, b, c, p);
	...
	lines = upper_hull(s1, n1, p1, pmax, lines, num_line, capacity);

	separate_points(points, num_point, pmax, pn, s1, s2, &n1, &n2);
	lines = upper_hull(s1, n1, pmax, pn, lines, num_line, capacity);
...
// p1, p2로 만들어지는 직선에 대해,
// ax+by-c < 0이면 1, ax+by-c > 0이면 -1 반환
int ccw(t_point p1, t_point p2, t_point p3)
{
