...
// 3. 힙에 삽입 (minheap 구성)
// 4. 2개의 최소값을 갖는 트리 추출
// 5. 두 트리를 결합 후 새 노드에 추가 (새 노드의 data는 임의로 초기화)
// 6. 새 트리를 힙에 삽입
...
tNode *make_huffman_tree( int *ch_freq)
{
	...
	while(heap->last > 0)
	{
		...
		tNode *merged = newNode(0, n1->freq + n2->freq);
		merged->left = n1;
		merged->right = n2;
...
// traverse_tree 함수 호출
// e.g. codes[a] = '0001', codes[b] = '1101011', ...
...
{
	int ch;
	int encoded_characters = 0;
	while((ch = fgetc(infp)) != EOF)
	{
		fputs(codes[ch], outfp);
		encoded_characters += strlen(codes[ch]);
	}

	return encoded_characters / 8;
}
...
void decoding( tNode *root, FILE *infp, FILE *outfp)
{
	...
	while((ch = fgetc(infp)) != EOF)
	{
		if(ch == '0') trav_ptr = trav_ptr->left;
		else trav_ptr = trav_ptr->right;

		if(!trav_ptr->left && !trav_ptr->right)
		{
			fputc(trav_ptr->data, outfp);
			trav_ptr = root;
		}
	}
}
